...
这个getWindow 拿到的window对象，对于activity来说就是 PhoneWindow （这点，目前是通过任玉刚的书籍，android开发艺术，初步确定的，  
...
中的内部实现细节。
 ...
 这个方法体，也较长，有几个重要的点，可以把我们一些知识点，连接起来。
1.  把decorView 父View标记为  ViewRootImp
view.assignParent(this);  
这样之后，activity的view 调用 requestLayout 就可以触动到 ViewRootImp了。  
2. 开始整个View 二叉树的绘制。
requestLayout(); 方法源码如下  
...
scheduleTraversals();方法就是整个view 二叉树 绘制/刷新的发起点。晚点我们再跟源码看下去。


# 具体理解下整个view二叉树，绘制/刷新过程。  
先看看View 的requestLayout 源码。  
...
源码注解的意思，就是， 如果有一些东西改变了，调用此方法，会安排一次layout过程。当view二叉树，正在  
layout过程中，不应该调用此方法。 如果正在layout，又调用了这个方法，那么这个layout请求会放在下一次layout过程  
或者放在layout过程的尾部。**有点不懂这个意思，以及实现**  
 看代码得知，如果ViewRootImpl 正在layout过程中，就会把这个Layout请求保存起来。  
 这里从viewRoot.requestLayoutDuringLayout(this)，看出。源码如下  
 ...
 如果不是在layout过程中的话，就会调用 mParent.requestLayout(); ViewGroup 并没有复写此方法。  
 意味着任何一个子View，调用requestLayout 都是会一直往上调用 requestLayout。而我们的 顶级mParent  
 是 ViewRootImp。ViewRootImp 的 requestLayout 方法，已经在上面提过了，终究就是调用scheduleTraversals（）方法。  
 这个方法，最终是调用 performTraversals() 方法来出发整个view绘制流程的。这个方法体，巨大。大概的代码逻辑如下  
 ...
 从注解以及变量名称，以及代码逻辑看， 如果这个window不处于stop状态，或者需要进行下一次draw，就进入  
 测量measure流程。
